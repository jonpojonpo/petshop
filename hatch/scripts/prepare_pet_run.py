#!/usr/bin/env python3
"""Create a pet run folder, prompts, and imagegen job manifest."""
...
STATE_REQUIREMENTS = {
    ...
    "waiting": [
        "Show that the agent needs approval, help, or user input through an expectant asking pose.",
...
def infer_description(args: argparse.Namespace, reference_paths: list[Path]) -> str:
    ...
    if args.pet_notes.strip():
        return sentence(f"A compact pet: {args.pet_notes}")
    if args.brand_name.strip():
        return sentence(f"A compact pet inspired by {args.brand_name}")
    if reference_paths:
        return "A compact pet based on the provided reference image."
    return "A compact original pet ready for animation."
...
def base_pet_prompt(args: argparse.Namespace) -> str:
    ...
    return f"""Create one clean full-body reference sprite for pet {args.display_name}.
...
def row_prompt(args: argparse.Namespace, state: str, row: int, frames: int, purpose: str) -> str:
    ...
    return f"""Create one horizontal animation strip for pet `{args.pet_id}`, state `{state}`.
...
def retry_row_prompt(
    ...
    state_prompt = STATE_PROMPTS[state]
    state_requirements = "\n".join(f"- {line}" for line in STATE_REQUIREMENTS[state])
    return f"""Create pet row `{state}` for `{args.pet_id}`: exactly {frames} full-body frames in one horizontal strip on flat pure {chroma_name} {chroma_key}.
...
def look_row_prompt(
    ...
    return f"""Create one horizontal look-direction strip for pet `{args.pet_id}`, atlas row {row}.
...
def retry_look_row_prompt(
    ...
    chroma_key = args.chroma_key["hex"]
    chroma_name = args.chroma_key["name"]
    return f"""Create v2 pet look row {row} for `{args.pet_id}` as exactly 8 full-body frames in this order: {direction_list}.
...
def look_cardinal_prompt(args: argparse.Namespace) -> str:
    chroma_key = args.chroma_key["hex"]
    chroma_name = args.chroma_key["name"]
    return f"""Create one horizontal four-cardinal anchor strip for pet `{args.pet_id}`.
...
def look_cardinal_repair_prompt(
    ...
    return f"""Repair one cardinal anchor for pet `{args.pet_id}`: `{label}` means looking {expected_direction}.
    ...
    for state, _row, frames, _purpose in ROWS:
        ...
        if state == "running-left":
            ...
            derivation_policy = {
                ...
                "fallback_generation_skill": "image",
            }
...
def main() -> None:
    ...
    request = {
        ...
        "primary_generation_skill": "image",
    }
