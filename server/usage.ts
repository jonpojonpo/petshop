...
export async function accountUsage() {
  ...
  pending = (async () => {
    ...
    const unwrap = (r: PromiseSettledResult<any>) =>
      r.status === "fulfilled"
        ? r.value
        : { available: false, error: r.reason.message };
    cached = {
...
async function codexUsage() {
  const env = cleanEnvironment("subscription");
  env.CODEX_HOME = CODEX_HOME;
  const rpc = new RpcProcess(
    process.execPath,
    [path.join(ROOT, "node_modules/@openai/codex/bin/codex.js"), "app-server"],
    { cwd: ROOT, env },
  );
  try {
    await rpc.request(
      "initialize",
      { clientInfo: { name: "petshop-usage", version: "0.1.0" } },
      20_000,
    );
    ...
    if (account.account?.type !== "chatgpt")
      return {
        available: false,
        error: "No ChatGPT subscription login detected.",
      };
    const limits = await rpc.request("account/rateLimits/read", {}, 20_000);
    const source = limits.rateLimitsByLimitId
    ...
    const windows = source
      ...
      .flatMap((x: any) =>
        [x.primary, x.secondary]
          .filter(Boolean)
          .map((w: any) => ({
            name: `${x.limitName || x.limitId || "Codex"} · ${w.windowDurationMins === 300 ? "5 hours" : w.windowDurationMins === 10080 ? "7 days" : `${w.windowDurationMins || "?"} min`}`,
...
async function claudeUsage() {
  ...
  const sheet = normalize({
    name: "Usage",
    description: "Account telemetry",
    model: "opus",
    sandbox_mode: "read-only",
    petshop: { harness: "claude", billing: "subscription" },
  });
  const pet = {
    ...sheet,
    id: "usage",
  ...
  const h = await createHarness(pet, ROOT, "account-usage");
  let text = "";
  try {
    h.rpc.onNotification = (method, p) => {
      if (
    ...
    const session = await h.rpc.request(
      "session/new",
      { cwd: ROOT, mcpServers: [], _meta: h.sessionMeta },
      20_000,
    );
    await h.rpc.request(
      "session/prompt",
    ...
    return {
      ...
      error: windows.length
        ? undefined
        : "Claude did not report usage windows. Open /usage in Claude Code.",
      source: "Claude Code /usage (no model inference)",
    };
  } finally {
    h.rpc.close();
  }
