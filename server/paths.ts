...
export const STATE = path.resolve(
  process.env.PETSHOP_STATE || path.join(ROOT, ".petshop"),
);
export const CODEX_HOME = path.resolve(
  process.env.PETSHOP_CODEX_HOME ||
...
export async function ensureDirs() {
  await Promise.all(
    [STATE, AGENTS, path.join(STATE, "bodies"), path.join(STATE, "runs")].map(
...
export function assertId(id: string) {
  if (!validId(id))
    throw new Error("Use letters, numbers, underscores or hyphens for the ID.");
}
